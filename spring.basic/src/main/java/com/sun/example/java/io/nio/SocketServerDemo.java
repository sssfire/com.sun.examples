package com.sun.example.java.io.nio;
...
public class SocketServerDemo {
     ...
     * 日志
     */
    private static final Log LOGGER = LogFactory.getLog(SocketServerDemo.class);
            ...
            while(true) {
                ...
                while(selecionKeys.hasNext()) {
                    ...
                    if(readyKey.isValid() && readyKey.isAcceptable()) {
                    	SocketServerDemo.LOGGER.info("======channel通道已经准备好=======");
                        /*
                    	...
                    	SocketServerDemo.LOGGER.info("======socket channel 建立连接=======");
                    } else if(readyKey.isValid() && readyKey.isReadable()) {
                    	SocketServerDemo.LOGGER.info("======socket channel 数据准备完成，可以去读==读取=======");
                        readSocketChannel(readyKey);
                    }
                }
            }
        } catch(Exception e) {
        	SocketServerDemo.LOGGER.error(e.getMessage() , e);
        } finally {
            serverSocket.close();
        }
    }
    ...
    private static void readSocketChannel(SelectionKey readyKey) throws Exception {
        ...
        if(URLDecoder.decode(message.toString(), "UTF-8").indexOf("over") != -1) {
            ...
            SocketServerDemo.LOGGER.info("端口:" + resoucePort + "客户端发来的信息======message : " + message);
            ...
            SocketServerDemo.LOGGER.info("端口:" + resoucePort + "客户端发来的完整信息======completeMessage : " + URLDecoder.decode(completeMessage.toString(), "UTF-8"));
        	...
        	SocketServerDemo.LOGGER.info("端口:" + resoucePort + "客户端信息还未接受完，继续接受======message : " + URLDecoder.decode(message.toString(), "UTF-8"));
